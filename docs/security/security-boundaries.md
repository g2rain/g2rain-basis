...
- `credentialCiphertext` 即使不是明文，也按敏感数据限制传播；管理列表与标准详情不得返回可复用密文。
- `tokenHash`（API Key 的 SHA-256 摘要）不可直接作为请求凭证，但属稳定密钥指纹：允许经鉴权后的 `PersonalStaticAccessToken` 列表/分页等接口返回；日志、审计与无关事件中仍不得随意扩散，且不得记录原始 API Key。

## 高风险操作

- 租户开通、IdP 全量同步、企业授权撤销、密码更新、角色/控制单元变更和静态令牌管理需要明确权限、审计、幂等及失败恢复。
- 个人静态访问令牌的全量查询（`/list_all`、`/page_all`）与代建（`/save_for_user`）走独立路径，由 Gateway 控制单元鉴权；服务端仅做本人作用域与本租户数据隔离，不依赖 `isAdminUser`。
- 个人静态访问令牌状态修改（`POST /{id}/status`）允许运营公司跨租户修改；非运营公司仅可修改本机构令牌。谁能调用由 Gateway 控制单元鉴权，服务端不依赖 `isAdminUser`。
