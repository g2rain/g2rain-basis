 ...
 * @since 2026/1/10
 */
@Schema(description = "控制单元状态枚举(发布状态)")
public enum ControlUnitStatus {

    /**
     * 已发布状态，表示控制单元已经正式发布并且客户可以使用。
     */
    @Schema(description = "已发布")
    PUBLISHED,

    /**
     * 未发布状态，表示控制单元尚未正式发布，无法供客户使用。
     */
     ...
     * </p>
     *
     * @param name 枚举名称字符串，表示控制单元状态的名称。
     * @throws BusinessException 如果指定的枚举名称无效，抛出此异常，表示参数值无效。
     */
    public static ControlUnitStatus fromName(String name) {
        if (Strings.isNotBlank(name)) {
            // 遍历所有枚举值，检查是否与传入的名称匹配
            for (ControlUnitStatus status : values()) {
                // 如果找到匹配的枚举，直接返回
